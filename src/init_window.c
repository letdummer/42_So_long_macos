...
/*   Updated: 2025/06/06 17:50:16 by ldummer-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void    ft_render_hud(t_game *game)
{
	...
	while (y < HUD_HEIGHT)
	{
		...
		{
			mlx_pixel_put(game->mlx_connection, game->mlx_wind, x, y, HUD_COLOR);
			x++;
		}
		y++;
	}
	moves = ft_strjoin("Moves: ", ft_itoa(game->moves));
	collectibles = ft_strjoin("Collectibles: ", ft_strjoin(ft_itoa(game->collected_items), 
